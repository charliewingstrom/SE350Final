...
final class InventorySet implements Inventory {
  ...
  public Iterator<Record> iterator(Comparator<Record> comparator) {
    // TODO  
	List<Record> a = new ArrayList<Record>(_data.values());
  ...
  Record checkOut(Video video) {
	...
	if (!(this._data.containsKey(video))) throw new IllegalArgumentException("Video has no Record");
	else if (this.get(video).numOut()==this.get(video).numOwned()) throw new IllegalArgumentException("All copies are already out");
	Record prevRecord = this.get(video);
	Record r = new RecordObj(video, this.get(video).numOwned(), this.get(video).numOut()+1,this.get(video).numRentals()+1);
  ...
  Record checkIn(Video video) {
    // TODO  
	if (!(this._data.containsKey(video))) throw new IllegalArgumentException("Video has no Record");
	else if (this.get(video).numOut()<1) throw new IllegalArgumentException("No copies are currently out");
	Record prevRecord = this.get(video);
	Record r = new RecordObj(video, this.get(video).numOwned(), this.get(video).numOut()-1,this.get(video).numRentals());
	this._data.put(video,r);
	return prevRecord;
  }
  ...
  Map<Video, Record> clear() {
    // TODO 
	Map<Video, Record> prevInventory = (Map<Video, Record>) ((HashMap<Video, Record>) this._data).clone();
	this._data.clear();
